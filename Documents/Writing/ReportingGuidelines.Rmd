...
```{r}
model = lm(BMI ~ weight + gender, data = diabetes)
summary(model)
```
...
1. The answer should succintly address the question being asked.  What are we intersting in determining?

2.  Optionally, the answer should include a plot if it aids in the understanding of the results.  Tables are nice, but plots are better.  Plots should always be captioned with an informative title or description.  Try to be concious of color here.  Many people are color blind, and choosing a poor color pallete means they would not be able to understand your plot. Consider using different shapes, or a color pallete which is able to be read by a color blind person.  Again, this is optional.
...
>*I used linear regression to perform the estimation. I estimate that males have, on average, 10.6 fewer BMI points (95% CI: (8.8, 12.4)) than women of the same weight.*
...
Good answers to statistical answers are dense in information while being easy to read and short.  It takes practice to become good at this sort of thing, but it pays in spades.
